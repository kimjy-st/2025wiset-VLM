...
# ============== 설정 ==============
# 예) https://github.com/USER/REPO/blob/main/mos_results/ 또는 /tree/main/mos_results/
GITHUB_JSONL_URL = st.secrets.get("GITHUB_JSONL_URL", "")
# 예) https://github.com/USER/REPO/blob/main/mapping.csv
VIDEO_MAPPING_CSV_URL = st.secrets.get("VIDEO_MAPPING_CSV_URL", "")
# 선택: GitHub Personal Access Token (classic read 권한이면 충분)
GITHUB_TOKEN = st.secrets.get("GITHUB_TOKEN", "")  # 없으면 비인증(레이트리밋 낮음)
...
def github_to_raw(url: str) -> str:
    """GitHub blob/tree URL → raw.githubusercontent.com URL로 변환"""
        ...
        if r.status_code == 200:
            ...
            for f in data:
                if f.get("type") == "file" and str(f.get("name", "")).endswith(".jsonl"):
                    files.append((f["name"], f["download_url"]))  # RAW 다운로드 URL
            return files
            ...
            break
    raise RuntimeError(f"GitHub API 요청 실패: {last.status_code if last else '??'}, {last.text[:200] if last else ''}")
...
def load_video_mapping_csv(url_or_page: str) -> pd.DataFrame:
    """GitHub의 mapping.csv 로드 (blob/tree URL 허용)"""
...
def build_video_url(video_path: str, mapping_df: Optional[pd.DataFrame]) -> Tuple[Optional[str], str]:
    """
    ...
    candidates = [f"{stem}__cv2.mp4" if ext else f"{base}__cv2.mp4", base]
...
if jsonl_files:
    ...
    if api_error:
        ...
        if not GITHUB_TOKEN:
            st.sidebar.info("✅ 해결: Streamlit Secrets에 GITHUB_TOKEN을 넣으면 레이트 리밋이 완화됩니다.")
...
if selected_mode == "GitHub 폴더 목록":
    ...
    selected_jsonl_url = dict(jsonl_files)[selected_name]  # RAW URL
elif selected_mode == "URL 직접 입력":
    selected_jsonl_url = st.sidebar.text_input(
        "JSONL 파일 RAW URL",
        value="",
        placeholder="https://raw.githubusercontent.com/<user>/<repo>/<branch>/mos_results/file.jsonl",
    )
...
with col1:
    ...
    if video_url is None:
        ...
        if video_url.startswith("gdrive:"):
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.link_button("새 창에서 열기", f"https://drive.google.com/file/d/{fid}/view?usp=sharing")
            with c2:
                st.link_button("다운로드", f"https://drive.google.com/uc?export=download&id={fid}")
        else:
            st.video(video_url)
